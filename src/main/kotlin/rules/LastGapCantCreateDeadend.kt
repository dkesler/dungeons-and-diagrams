...
class LastGapCantCreateDeadend: Rule {
    ...
    private fun wouldBeDeadEndAsGap(rowIdx: Int, colIdx: Int, board: Board, checkingRow: Boolean): Boolean {
        val verticalNeighbors = verticalNeighbors(rowIdx, colIdx, board.grid.count())
        val horizontalNeighbors = horizontalNeighbors(rowIdx, colIdx, board.grid[0].count())
        ...
        val neighborWallCount = untouchedNeighbors.count{ board.grid[it.first][it.second].eq(CellType.WALL) } +
                touchedNeighbors.count{ board.grid[it.first][it.second].canBe(CellType.WALL) }
