        ...
        fun colRule(col: List<Point>, colIdx: Int): Rule.Check? {
            if (wallsRemaining(board.colReqs[colIdx], col) == 1) {
                ...
                for (rowIdx in board.grid.rows) {
                    if (!col[rowIdx].type.known && col[rowIdx].type.canBe(Type.WALL) && wouldCreateDeadEndAsWall(rowIdx, col, board)) {
                        changeFromWall.add(Point(rowIdx, colIdx, TypeRange(col[rowIdx].type.types - Type.WALL)))
                    }
                }
                if (changeFromWall.isNotEmpty()) {
                    return Rule.Check(
