    ...
    companion object {
        fun fromBox(box: Box, board: Board): WallBoundBox {
            ...
            for (rowIdx in (box.minRow..box.maxRow)) {
                ...
                val wbbRowMax = min(
                    row.filterIndexed{ colIdx, space -> box.contains(rowIdx, colIdx) && space.canBe(CellType.WALL)}.count(),
                    board.rowReqs[rowIdx] - row.filterIndexed{ colIdx, space -> !box.contains(rowIdx, colIdx) && space.eq(CellType.WALL)}.count()
                )
            ...
            for (colIdx in (box.minCol..box.maxCol)) {
                ...
                val wbbColMax = min(
                    col.filterIndexed{ rowIdx, space -> box.contains(rowIdx, colIdx) && space.canBe(CellType.WALL)}.count(),
                    board.colReqs[colIdx] - col.filterIndexed{ rowIdx, space -> !box.contains(rowIdx, colIdx) && space.eq(CellType.WALL)}.count()
                )
