    ...
    companion object {
        fun fromBox(box: Box, board: Board): WallBoundBox {
            ...
            for (rowIdx in (box.minRow..box.maxRow)) {
                ...
                val wbbRowMin = board.rowReqs[rowIdx] - row.filterIndexed{ colIdx, space -> !box.contains(rowIdx, colIdx) && (space == Space.UNKNOWN || space == Space.WALL)}.count()
                //how many walls can we cram in the box.  it will be limited by the number of available slots in the row and the number of required walls for the row minus the number of placed walls
                val wbbRowMax = min(
                    row.filterIndexed{ colIdx, space -> box.contains(rowIdx, colIdx) && (space == Space.UNKNOWN || space == Space.WALL)}.count(),
                    board.rowReqs[rowIdx] - row.filterIndexed{ colIdx, space -> !box.contains(rowIdx, colIdx) && space == Space.WALL}.count()
                )
            ...
            for (colIdx in (box.minCol..box.maxCol)) {
                ...
                val wbbColMin = board.colReqs[colIdx] - col.filterIndexed{ rowIdx, space -> !box.contains(rowIdx, colIdx) && (space == Space.UNKNOWN || space == Space.WALL)}.count()
                ...
                val wbbColMax = min(
                    col.filterIndexed{ rowIdx, space -> box.contains(rowIdx, colIdx) && (space == Space.UNKNOWN || space == Space.WALL)}.count(),
                    board.colReqs[colIdx] - col.filterIndexed{ rowIdx, space -> !box.contains(rowIdx, colIdx) && space == Space.WALL}.count()
                )
